...
export const createProduct = async (req: Request, res: Response) => {
    try {
        ...
        if (req.files && (req.files as any).length > 0) {
            const uploadPromises = (req.files as any).map((file: any) => {
                return new Promise((resolve, reject) => {
                    const uploadStream = cloudinary.uploader.upload_stream({ folder: "ecom-app-native/products" }, (error, result) => {
        ...
        if (images.length === 0) {
            return res.status(400).json({ success: false, message: "Please upload at least on image" });
        }
