...
export default function Header({ title, showBack, showSearch, showCart, showMenu, showLogo }: HeaderProps) {
    ...
    const { itemsCount } = { itemsCount: 6 }
    return (
        <View className='flex-row item-center justify-between px-4 py3 bg-white'>
            {/* left side */}
            <View className='flex-row items-center flex-1'>
                ...
                {showLogo ? (
                    <View className='flex-1'>
                        <Image src={require("@/assets/logo.png")} style={{width: '100%', height: 24}} resizeMode='contain' />
                    </View>
                ) : title && (
                    ...
                    <TouchableOpacity onPress={() => router.push('/(tabs)/cart')}>
                        <View className='relative'>
                            ...
                            <View className='absolute -top-1 -right-1 bg-accent w-4 h-4 rounded-full items-center justify-center'>
                                <Text className='text-white text-[10px] font-bold'>{itemsCount}</Text>
                            </View>
                        </View>
                    </TouchableOpacity>
                )}
